...
void interpret_goto_jit2(const struct _vliw *vliw, int len) {
    ...
    struct _vliw_jit *vliw_jit = malloc(len*sizeof(struct _vliw_jit)); // +1 exit
    for (int i=0; i<len; i++) {
	vliw_jit[i].instr = instruction_table[vliw[i].opcode & 0x0f];
	vliw_jit[i].outptr = vliw[i].outptr;
	vliw_jit[i].m1ptr = vliw[i].m1ptr;
	vliw_jit[i].m2ptr = vliw[i].m2ptr;
    }
