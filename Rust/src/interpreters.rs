...
pub fn interpret_switch(vliw_enum: Vec<VliwEnum>, mut reg: Vec<f64>) -> Vec<f64> {
	...
	match vliw_enum[progct] { // hint: NOP <== and 0, 0, 0  ( out = reg0 & reg0 )
	    ...
	    VliwEnum::Incjl (outptr, m1ptr, m2ptr)=> {reg[outptr]+=1.; if reg[outptr] <  reg[m2ptr] {progct+=reg[m1ptr] as usize}},
	    VliwEnum::Decjge(outptr, m1ptr, m2ptr)=> {reg[outptr]-=1.; if reg[outptr] >= reg[m2ptr] {progct+=reg[m1ptr] as usize}},

	    VliwEnum::Je    (outptr, m1ptr, m2ptr)=> if reg[outptr] == reg[m2ptr] {progct+=reg[m1ptr] as usize},
	    VliwEnum::Jne   (outptr, m1ptr, m2ptr)=> if reg[outptr] != reg[m2ptr] {progct+=reg[m1ptr] as usize},
...
fn instr_incjl (_callstack: &mut Vec<usize>, out:f64, m1: f64, m2: f64) -> (f64, usize) { let o = out+1.; if o < m2 { (o, m1 as usize) } else {(o, 0)} }
fn instr_decjge(_callstack: &mut Vec<usize>, out:f64, m1: f64, m2: f64) -> (f64, usize) { let o = out-1.; if o >= m2 { (o, m1 as usize) } else {(o, 0)} }
fn instr_je    (_callstack: &mut Vec<usize>, out:f64, m1: f64, m2: f64) -> (f64, usize) { if out == m2 { (out, m1 as usize) } else { (out, 0)} }
fn instr_jne   (_callstack: &mut Vec<usize>, out:f64, m1: f64, m2: f64) -> (f64, usize) { if out != m2 { (out, m1 as usize) } else { (out, 0)} }
